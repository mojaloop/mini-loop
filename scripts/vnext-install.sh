#!/usr/bin/env bash
# vnext-install.sh
#               - install mojaloop vNext in a fast and simple as possible manner for demos , test and education 
#                 the install_ml option of this script will git clone the latest version of mojaloop vNext repo 
#                 master branch  and then make any needed local modifications to these charts to enable Mojaloop vNext 
#                 to deploy and run in current latetest kubernetes versions.  
#                      
# Author Tom Daly 
# Date May 2023
...
Options:
-m mode ............ install_ml|check_ml|delete_ml|install_infra|delete_infra
...
-n namespace ....... the namespace to deploy mojaloop vNext into 
-l logfilename ..... the name of the .log and .err files to create in /tmp
-f force ........... force the cloning and updating of the vNext repo
...
while getopts "fd:t:n:m:l:hH" OPTION ; do
   case "${OPTION}" in
        f)  force="true"
        ;; 
        t)  tsecs="${OPTARG}"
