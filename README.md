...
## known issues
1. Again mini-loop deployment of Mojaloop has only been tested properly with ubuntu as specified previously.
2. Not contradicting point 1. above BUT the k3s option should(?) work on linux distros other than Ubuntu. It has been tested on fedora36 where there were some issues around docker volumes that are not yet fully debugged but it is unclear that these docker issues will occur on other redhat type linux distros as they do not occur on Ubuntu where k3s with docker seems to work just fine. [Note if you try mini-loop k3s on Linux other than Ubuntu please let me know (tdaly61@gmail.com) ]
3. The versions of kubernetes are currently hardcoded this is done os that the mojaloop helm charts and nginx and networking values are reliably set and any known container runtime issues are avoided.  To be clear currently mini-loop ignores the commandline version flag and uses only  :-
    * MicroK8s : v1.20
    * k3s      : v1.21
