#!/usr/bin/env bash
# for k3s ubuntu / mojaloop installation 
...
echo "export KUBECONFIG=/home/ubuntu/k3s.yaml" >> /home/ubuntu/.bash_profile
echo "source <(kubectl completion bash)" >> /home/ubuntu/.bashrc # add autocomplete permanently to your bash shell.
echo "alias k=kubectl " >> /home/ubuntu/.bashrc
echo "complete -F __start_kubectl k " >> /home/ubuntu/.bashrc
echo 'alias ksetns="kubectl config set-context --current --namespace"'  >> /home/ubuntu/.bashrc
echo "alias ksetuser=\"kubectl config set-context --current --user\""  >> /home/ubuntu/.bashrc
...
su - ubuntu -c "helm repo add mojaloop http://mojaloop.io/helm/repo/"
#su - ubuntu -c "helm repo add stable https://charts.helm.sh/stable"
#su - ubuntu -c "helm repo add incubator https://charts.helm.sh/incubator"
su - ubuntu -c "helm repo add kiwigrid https://kiwigrid.github.io"
su - ubuntu -c "helm repo add elastic https://helm.elastic.co"
...
#su - ubuntu -c "helm repo add kong https://charts.konghq.com" 
su - ubuntu -c "helm repo update"
#su - ubuntu -c "export KUBECONFIG=~/k3s.yaml; helm list"
su - ubuntu -c "helm list"
su - ubuntu -c "helm repo list"
...
su - ubuntu -c "helm install --wait --timeout 300s ingress-nginx ingress-nginx/ingress-nginx --version=\"3.33.0\"  "

echo "**** Infrastructure to install and run Mojaloop should now be installed and running ****"
### notes and things for later #####$

# install calico 
#su - ubuntu -c "kubectl create -f https://docs.projectcalico.org/manifests/tigera-operator.yaml"
#su - ubuntu -c "kubectl create -f https://docs.projectcalico.org/manifests/custom-resources.yaml"
